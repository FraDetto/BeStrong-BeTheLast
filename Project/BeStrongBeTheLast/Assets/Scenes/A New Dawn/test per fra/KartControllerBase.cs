...
public class KartControllerBase : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            int dir = Input.GetAxis("P1Horizontal") > 0 ? 1 : -1;
            float amount = Mathf.Abs((Input.GetAxis("P1Horizontal")));
            Steer(dir, amount);
        }

        //Drift
        if (Input.GetButtonDown("P1Drift") && !drifting && Input.GetAxis("P1Horizontal") != 0)
        {
            drifting = true;
            driftDirection = Input.GetAxis("P1Horizontal") > 0 ? 1 : -1;
        ...
        if (drifting)
        {
            float control = (driftDirection == 1) ? ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, 0, 2) : ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, 2, 0);
            float powerControl = (driftDirection == 1) ? ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, .2f, 1) : ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, 1, .2f);
        ...
        if (Input.GetButtonUp("P1Drift") && drifting)
        {
            Boost();
        }
        ...
        if (!drifting)
        {
            kartModel.localEulerAngles = Vector3.Lerp(kartModel.localEulerAngles, new Vector3(0, 90 + (Input.GetAxis("P1Horizontal") * 15), kartModel.localEulerAngles.z), .2f);
        }
        else
        {
            float control = (driftDirection == 1) ? ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, .5f, 2) : ExtensionMethods.Remap(Input.GetAxis("P1Horizontal"), -1, 1, 2, .5f);
            kartModel.parent.localRotation = Quaternion.Euler(0, Mathf.LerpAngle(kartModel.parent.localEulerAngles.y,(control * 15) * driftDirection, .2f), 0);
        }

        //b) Wheels
        frontWheels.localEulerAngles = new Vector3(0, (Input.GetAxis("P1Horizontal") * 15), frontWheels.localEulerAngles.z);
        frontWheels.localEulerAngles += new Vector3(0, 0, sphere.velocity.magnitude/2);
        backWheels.localEulerAngles += new Vector3(0, 0, sphere.velocity.magnitude/2);

        //c) Steering Wheel
        steeringWheel.localEulerAngles = new Vector3(-25, 90, ((Input.GetAxis("P1Horizontal") * 45)));
